...
public class DMAController extends AbstractHardwareComponent implements IOPortCapable
{
    ...
    public void reset()
    {
        for (DMAChannel dmaChannel : dmaChannels) dmaChannel.reset();
    ...
    public int[] ioPortsRequested()
    {
        ...
        for (int aPagePortList : pagePortList) {
            temp[j++] = pageLowBase + aPagePortList;
            if (pageHighBase >= 0)
                temp[j++] = pageHighBase + aPagePortList;
        }
