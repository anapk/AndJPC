...
public abstract class FpuState implements Hibernatable
{
    ...
    final static int STACK_DEPTH = 8;

    public static final int FPU_PRECISION_CONTROL_SINGLE = 0;
    static final int FPU_PRECISION_CONTROL_DOUBLE = 2;
    ...
    static final int FPU_TAG_VALID = 0;
    static final int FPU_TAG_ZERO = 1;
    static final int FPU_TAG_SPECIAL = 2;
    static final int FPU_TAG_EMPTY = 3;
    ...
    int top; // top of stack pointer (3 bits)
    // control word
    ...
    boolean infinityControl; // legacy:  not really used anymore
    ...
    // other registers
    long lastIP; // last instruction pointer
    long lastData; // last data (operand) pointer
    int lastOpcode; // 11 bits
