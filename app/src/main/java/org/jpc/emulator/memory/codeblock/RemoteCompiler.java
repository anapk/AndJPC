...
{
    private static final String JPC_DYNAMIC_URI_STRING = "/JPCDynamicClasses/";
    private static final URI JPC_DYNAMIC_URI = URI.create(JPC_DYNAMIC_URI_STRING);
        ...
        public void run()
        {
            while (true) 
            {
                ...
                try
                {
                    ...
                    int hash = -1;
                    for (byte aRawCodeBlock : rawCodeBlock) {
                        hash ^= aRawCodeBlock;
                        hash *= 31;
                    }
