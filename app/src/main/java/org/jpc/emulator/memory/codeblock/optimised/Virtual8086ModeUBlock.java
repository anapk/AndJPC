...
// 	    } break;
    ...
    private void call_o16_a16(short target)
    {
    ...
    private void ret_iw_o16_a16(short data)
    {
	ret_o16_a16();
	cpu.esp = (cpu.esp & ~0xffff) | ((cpu.esp + data) & 0xffff);
    }

    private void ret_far_o16_a16()
    {
	// TODO:  supposed to throw SS exception
	// "if top 6 bytes of stack not within stack limits"
	cpu.eip = cpu.ss.getWord(cpu.esp & 0xffff) & 0xffff;
    ...
    private void ret_far_iw_o16_a16(short offset)
    {
	// TODO:  supposed to throw SS exception
	// "if top 6 bytes of stack not within stack limits"
	ret_far_o16_a16();
    ...
    private void push_o16(short data)
    {
        if (cpu.ss.getDefaultSizeFlag()) {
    ...
    private void push_o32(int data)
    {
        if (cpu.ss.getDefaultSizeFlag()) {
    ...
    private void jump_far_o16(int targetEIP, int targetSelector)
    {
	cpu.eip = targetEIP;
	cpu.cs.setSelector(targetSelector);
    }

    private void jump_far_o32(int targetEIP, int targetSelector)
    {
	cpu.eip = targetEIP;
	cpu.cs.setSelector(targetSelector);
    }
    ...
    private void int_o16_a16(int vector, int position)
    {
	if ((cpu.getCR4() & Processor.CR4_VIRTUAL8086_MODE_EXTENSIONS) != 0)
...
// 		sub_o32_flags((0xffffffffl & dataOne) - (0xffffffffl & dataTwo), dataOne, dataTwo);
    ...
    private void insb_a16(int port)
    {
    ...
    private void insw_a16(int port)
    {
    ...
    private void insd_a16(int port)
    {
    ...
    private void rep_insb_a16(int port)
    {
	if (!checkIOPermissionsByte(port))
	    throw ProcessorException.GENERAL_PROTECTION_0;
    ...
    private void rep_insw_a16(int port)
    {
	if (!checkIOPermissionsShort(port))
	    throw ProcessorException.GENERAL_PROTECTION_0;
    ...
    private void rep_insd_a16(int port)
    {
	if (!checkIOPermissionsInt(port))
	    throw ProcessorException.GENERAL_PROTECTION_0;
...
// 	sub_o8_flags(data - input, data, input);
    ...
    private void scasw_a16(int data)
    {
	int addr = cpu.edi & 0xffff;
...
// 	sub_o16_flags(data - input, data, input);
    ...
    private void scasd_a16(int data)
    {
	int addr = cpu.edi & 0xffff;
...
// 		sub_o8_flags(data - input, data, input);
    ...
    private void repe_scasw_a16(int data)
    {
	int count = cpu.ecx & 0xffff;
	int addr = cpu.edi & 0xffff;
        boolean used = count != 0;
...
// 		sub_o16_flags(data - input, data, input);
    ...
    private void repe_scasd_a16(int data)
    {
	int count = cpu.ecx & 0xffff;
	int addr = cpu.edi & 0xffff;
        boolean used = count != 0;
...
// 		sub_o8_flags(data - input, data, input);
    ...
    private void repne_scasw_a16(int data)
    {
	int count = cpu.ecx & 0xffff;
	int addr = cpu.edi & 0xffff;
        boolean used = count != 0;
...
// 	bitwise_flags((byte)ax2);
    ...
    private int aam(int base) throws ProcessorException
    {
        int tl = 0xff & cpu.eax;
        if (base == 0) 
            throw ProcessorException.DIVIDE_ERROR;
...
// 	bitwise_flags((byte)cpu.eax);
    ...
    private void rep_sub_o32_flags(int used, int operand1, int operand2)
    {
        if (used == 0)
            return;
    ...
    private void rep_sub_o16_flags(int used, int operand1, int operand2)
    {
        if (used == 0)
            return;
    ...
    private void rep_sub_o8_flags(int used, int operand1, int operand2)
    {
        if (used == 0)
            return;
    ...
    private void out_o8(int ioport, int data)
    {
	if (checkIOPermissionsByte(ioport))
    ...
    private void out_o16(int ioport, int data)
    {
	if (checkIOPermissionsShort(ioport))
    ...
    private void out_o32(int ioport, int data)
    {
	if (checkIOPermissionsInt(ioport))
