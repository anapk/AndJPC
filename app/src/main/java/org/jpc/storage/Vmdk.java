...
public class Vmdk implements SeekableDataIO
{
    ...
    private static final int VMDK4HEADER_SIZE = 65536;
    ...
    public int readFully(byte[] content, int offset, int length) throws IOException
    {
        ...
        while (toRead > 0)
        {
            int indexInCluster = sectorNum % header.granularity;
    ...
    {
        ((int[])(header.l2Table.get(state.l2Offset)))[state.l2Index] = state.offset;
        ((int[])(header.l2BackupTable.get(header.gdPosition[state.l1Index])))[state.l2Index] = state.offset;
    }
    ...
    private void createVmdk(long length) throws IOException{       
        ...
        header.rgdPosition = new int[header.gtCount];
        header.gdPosition = new int[header.gtCount];
        header.l2Table = new HashMap();
        header.l2BackupTable = new HashMap();
        for(int i = 0, tmp = header.gdSize; i < header.gtCount; i++, tmp +=  header.gtSize){
    ...
    private void headerUpdate() throws IOException{
        ...
        bbuf.put(header.filler);
        bbuf.put(header.checkBytes[0]);
        bbuf.put(header.checkBytes[1]);
        bbuf.put(header.checkBytes[2]);
        bbuf.put(header.checkBytes[3]);
        ...
        for(int i=0; i<header.l2Table.size(); i++){
            bbuf = ByteBuffer.allocate(header.numberGtesPerGte * 4);
            bbuf.order(ByteOrder.LITTLE_ENDIAN);
            IntBuffer ibuf = bbuf.asIntBuffer();
            ibuf.put((int[])header.l2Table.get(header.rgdPosition[i]));
            address = header.rgdPosition[i] << 9;
            dio.writeFully(address, bbuf.array(), 0, header.numberGtesPerGte * 4);
            ibuf.clear();
            bbuf.clear();
        }
